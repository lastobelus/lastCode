...
while the Keychain prompt is open. If that delays local authentication requests,
the desktop client keeps retrying for one hour. You can leave the build or
install unattended, return to handle a prompt, and continue without rebuilding,
reinstalling, or relaunching LastCode.
