...
it.effect("routes pinned flags through each real thread leaf parser", () =>
  Effect.gen(function* () {
    ...
    for (const command of ["current", "list", "read", "send", "wait"] as const) {
    ...
    const unsupported = NodeChildProcess.spawnSync(result.wrapperPath, ["future-command"], {
      encoding: "utf8",
    });
    assert.strictEqual(unsupported.status, 64);
    assert.match(unsupported.stderr, /unsupported command 'future-command'/);
  }).pipe(Effect.scoped, Effect.provide(Layer.mergeAll(NodeServices.layer))),
);
