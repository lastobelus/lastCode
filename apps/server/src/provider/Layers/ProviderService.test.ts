...
describe("agent browser access", () => {
  ...
  it.effect("requests an MCP credential without preview when agent browser access is off", () =>
    Effect.gen(function* () {
  ...
  it.effect("requests an MCP credential when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, providerInstanceId: codexInstanceId, enablePreview: true },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );

  it.effect(
    "requests an MCP credential without preview when the project disables browser access",
    () =>
      Effect.gen(function* () {
        const threadId = asThreadId("thread-project-browser-off");
        const issued = yield* startSessionWith(true, threadId, false);
        assert.deepEqual(issued, [
          { threadId, providerInstanceId: codexInstanceId, enablePreview: false },
        ]);
      }).pipe(Effect.provide(NodeServices.layer)),
  );
