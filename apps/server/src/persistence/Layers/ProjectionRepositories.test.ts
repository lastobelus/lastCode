...
projectionRepositoriesLayer("Projection repositories", (it) => {
  ...
  it.effect("stores JSON for thread model options", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(yield* threads.listPendingWorktreeCleanup(), []);
      const pendingCleanupThread = {
        ...Option.getOrThrow(persisted),
        unsettledAt: "2026-03-24T00:02:00.000Z",
        activeOrderKey: "gm",
        persistent: 1,
        worktreeCleanup: {
          ...
          worktreePath: "/tmp/thread-null-options-worktree",
          queuedAt: "2026-03-24T00:02:00.000Z",
          blockedByThreadId: ThreadId.make("cleanup-owner"),
        },
      };
      yield* threads.upsert(pendingCleanupThread);
      assert.deepStrictEqual(yield* threads.listPendingWorktreeCleanup(), [pendingCleanupThread]);
    }),
  );
