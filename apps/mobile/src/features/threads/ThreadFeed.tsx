...
import { resolveThreadStatus } from "./thread-status";
...
function AgentMessageTimelineRow(props: {
  ...
  return (
    <Animated.View
      ...
      {...(isFreshTimestamp(message.createdAt) ? { entering: FadeIn.duration(220) } : {})}
    >
      <View
        ...
        <View className="min-w-0 flex-row flex-wrap items-center gap-1.5">
          ...
          {status ? (
            <View className="flex-row items-center gap-1">
              <Text className={cn("text-2xs", status.textClassName)}>●</Text>
